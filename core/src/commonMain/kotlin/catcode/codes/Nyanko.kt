...
//  *
//  * 相比于[MapNeko], [Nyanko]在实例化与较短猫猫码处理的消耗会更低，
//  * 但是无论如何[Nyanko.get]的速度也永远比不上hash表的速度。
//  * 但是对于直接通过字符串构建一个[Neko]来讲，[Nyanko]无疑是消耗更低的选择。
//  *
//  * [Nyanko]构建速度更快、资源消耗更少, 获取静态参数(例如[toString]、[size])的速度更快。
//  * [Nyanko]的构建、获取静态参数速度相比较于[MapNeko]有着几百倍的差距。
//  *
...
//      * 此处为保留原本的值不做转义。
//      */
//     override fun getWithoutDecode(key: String): String? = getParam(key)
//
//     /**
//      * 转化为可变参的[MutableNeko]
//      */
...
//             return this@Nyanko.code.contains(kv)
//         }
//
//         /**
//          * 查看是否包含所有的键值对
...
//          * 是否为空set
//          */
//         override fun isEmpty(): Boolean = this@Nyanko.empty
//
//         /**
//          * 键值对迭代器
//          */
...
//             return this@Nyanko.code.contains("$element$CAT_KV")
//         }
//
//         /**
//          * 是否contains all
...
//             if (empty) return false
//             // 判断有没有 '=element' 字符串
//             return this@Nyanko.code.contains("$CAT_KV${CatEncoder.encodeParams(element)}")
//         }
//
//         /**
...
//          */
//         @JvmStatic
//         fun byCode(code: String): Nyanko = Nyanko(code.trim())
//     }
//
// }
//
